...
hashUpdate ctx b
    | B.null b  = ctx
    | otherwise = trace "Crypto.Hash hashUpdate" $ hashUpdates ctx [b]
...
hashUpdates :: forall a ba . (HashAlgorithm a, ByteArrayAccess ba)
            ...
            -> Context a
hashUpdates c l
    | null ls   = trace "Crypto.Hash hashUpdates - null ls" c
    | otherwise = trace "Crypto.Hash hashUpdates - otherwise" $ Context $ 
        (trace "Crypto.Hash hashUpdates - otherwise - copyAndFreeze" $ B.copyAndFreeze c) $ 
        \(ctx :: Ptr (Context a)) -> mapM_ (\b -> (trace "Crypto.Hash hashUpdates - otherwise - withByteArray" $ B.withByteArray b) $ \d -> (trace "Crypto.Hash hashUpdates - otherwise - hashInternalUpdate" $ hashInternalUpdate) ctx d (fromIntegral $ B.length b)) ls
  where
