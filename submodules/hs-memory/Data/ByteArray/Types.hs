...
class ByteArrayAccess ba where
    ...
    copyByteArrayToPtr a dst = trace "Data.ByteArray Types class ByteArrayAccess - copyByteArrayToPtr" $ withByteArray a $ \src -> memCopy (castPtr dst) src (length a)
...
instance ByteArray Bytestring.ByteString where
    allocRet sz f = do
        fptr <- trace "Data.ByteArray Types instance ByteStrying allocRet" $ Bytestring.mallocByteString sz
...
instance Base.PrimType ty => ByteArrayAccess (Block.Block ty) where
    ...
    copyByteArrayToPtr ba dst = do
        mb <- trace "Data.ByteArray Types instance ByteArrayAccess Block.Block - copyByteArrayToPtr" $ Block.unsafeThaw (baseBlockRecastW8 ba)
...
instance (KnownNat n, Base.PrimType ty, Base.Countable ty n) => ByteArrayAccess (BlockN.BlockN n ty) where
    ...
    copyByteArrayToPtr bna = trace "Data.ByteArray Types instance ByteArrayAccess BlockN.BlockN - copyByteArrayToPtr" $ copyByteArrayToPtr (BlockN.toBlock bna)
...
instance Base.PrimType ty => ByteArrayAccess (Base.UArray ty) where
    ...
    copyByteArrayToPtr ba dst = trace "Data.ByteArray Types instance ByteArrayAccess Base.UArray - copyByteArrayToPtr" $ Base.copyToPtr ba (castPtr dst)
...
instance (Ord ty, Base.PrimType ty) => ByteArray (Block.Block ty) where
    allocRet sz f = do
        mba <- trace "Data.ByteArray Types instance ByteArray Block.Block allocRet" $ Block.new $ sizeRecastBytes sz Proxy
...
instance (Ord ty, Base.PrimType ty) => ByteArray (Base.UArray ty) where
    allocRet sz f = do
        mba <- trace "Data.ByteArray Types instance ByteArray Base.UArray allocRet" $ Base.new $ sizeRecastBytes sz Proxy
